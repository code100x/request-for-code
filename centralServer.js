...
const fs = require('fs');
const { generateBlockHash, publicKeyToAddress, validateBlock } = require('./blockchainUtils');
...
async function initializeBlockchain() {
    ...
    const genesisBlock = {
        ...
        transactions: [],
        nonce: 0,
        hash: generateBlockHash({ index: 0, previousHash: '0'.repeat(64), timestamp: genesisTimestamp, transactions: [], nonce: 0 })
    };
...
function adjustDifficulty() {
    ...
    if (averageBlockTime < desiredBlockTime) { // 30 seconds in milliseconds
        difficulty++;
